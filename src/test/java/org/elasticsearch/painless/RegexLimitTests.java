...
public class RegexLimitTests extends ScriptTestCase {
    ...
    private void setRegexLimitFactor(int factor) {
        Settings settings = Settings.builder().put(CompilerSettings.REGEX_LIMIT_FACTOR, factor).build();
        scriptEngine = new PainlessScriptEngine(scriptContexts(), settings.getEnabled(), settings.getFactor());
    }

    private void setRegexEnabled() {
        Settings settings = Settings.builder().put(CompilerSettings.REGEX_ENABLED, "true").build();
        scriptEngine = new PainlessScriptEngine(scriptContexts(), settings.getEnabled(), settings.getFactor());
    }
}
