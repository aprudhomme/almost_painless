...
public final class PainlessScriptEngine {
     ...
     * of {@link PainlessScriptEngine#PainlessScriptEngine(Map, CompilerSettings.RegexEnabled, int)}.
     */
    ...
    /**
     * Constructor. Uses default regex params.
     *
     * @param contexts whitelists by script type
     */
    public PainlessScriptEngine(Map<ScriptContext<?>, List<Whitelist>> contexts) {
        this(contexts, CompilerSettings.DEFAULT_REGEX_ENABLED, CompilerSettings.DEFAULT_REGEX_LIMIT_FACTOR);
    }
     ...
     *
     * @param contexts whitelists by script type
     * @param regexEnabled to what extent is regex enabled, see {@link CompilerSettings}
     * @param regexLimitFactor what is the max complexity of limited regex, see {@link CompilerSettings}
     */
    public PainlessScriptEngine(Map<ScriptContext<?>, List<Whitelist>> contexts, CompilerSettings.RegexEnabled regexEnabled, int regexLimitFactor) {
        if (regexLimitFactor < 1) {
            throw new IllegalArgumentException("regexLimitFactor must be >= 1");
        }
        defaultCompilerSettings.setRegexesEnabled(regexEnabled);
        defaultCompilerSettings.setRegexLimitFactor(regexLimitFactor);
    ...
    private CompilerSettings buildCompilerSettings(Map<String, String> params) {
        ...
        if (params.isEmpty()) {
            ...
            value = copy.remove(CompilerSettings.REGEX_ENABLED);
            if (value != null) {
                throw new IllegalArgumentException("[regex_enabled] can only be set on node startup.");
            }

            value = copy.remove(CompilerSettings.REGEX_LIMIT_FACTOR);
            if (value != null) {
                throw new IllegalArgumentException("[regex_limit_factor] can only be set on node startup.");
            }
